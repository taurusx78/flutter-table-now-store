...
class NoticeProvider extends GetConnect {
  // 알림 전제조회
  Future<Response> findAll(int id) async => await get('$host/store/$id/notice');

  // 알림 등록
  Future<Response> save(int id, Map<String, dynamic> data) async => await post(
  ...
  // 알림 삭제
  Future<Response> deleteById(int storeId, int noticeId) async => await delete(
        '$host/manager/store/$storeId/notice/$noticeId',
        headers: {'authorization': jwtToken ?? ''},
      );
}
